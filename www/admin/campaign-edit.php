...
function buildZoneFormSection(&$form, $campaign, $newCampaign)
{
    ...
    $rsZones = DBC::NewRecordSet($query_zones);    
    if (PEAR::isError($rsZones)) {
        return $rsZones;
    }        
    $aZoneAll = $rsZones->getAll();       
    
    if($newCampaign){
        $arrayActive="";
        foreach($aZoneAll as $aZone){   
            $arrayActive .=','.$aZone['zoneid'];
        }
    }else{
        ...
        $query_activezone = "
        SELECT 
            pza.zone_id,pza.placement_id,z.zoneid
        FROM
            ox_zones AS z LEFT JOIN ox_placement_zone_assoc AS pza ON z.zoneid=pza.zone_id
        WHERE
            pza.placement_id =$campaignId;
    ";    
        $rsZonesActive = DBC::NewRecordSet($query_activezone);    
            if (PEAR::isError($rsZonesActive)) {
                return $rsZonesActive;
            }        
         $aZoneActiveAll = $rsZonesActive->getAll();    
            $arrayActive="";
            foreach($aZoneActiveAll as $aZoneActive){  
                $arrayActive .=','.$aZoneActive['zone_id'];
            } 
    }    
     $form->addElement ( 'header', 'h_Zone','Select Zone');
    // $form->addElement ('html',NULL, '<script src="' . OX::assetPath() . '/js/adflex/jquery-1.8.3.min.js"></script>');
    $arrayCheckAll="";
        foreach($aZoneAll as $aZone){   
            $arrayCheckAll .=','.$aZone['zoneid'];
        }    
       ...
       $js='
           <script>
                ...
                jQuery(document).ready(function($) {              
                    ...
                    $(".boxrow input").click(function() {
                        var target_zone = $("#target_zone").val();
                        if($("."+$(this).attr("class")).is(":checked")){
                            target_zone=target_zone+","+this.value;
                            $("#target_zone").val(target_zone);
                         }else{
                            $("#target_zone").val(target_zone.replace(this.value,""));
                         }
                    });                   

                    $(".boxrow").click(function() {
                        var target_zone = $("#target_zone").val();
                            if(!$("."+this.id).is(":checked")){
                                target_zone=target_zone+","+$("."+this.id).val();
                                $("#target_zone").val(target_zone);
                             }else{
                                $("#target_zone").val(target_zone.replace(","+$("."+this.id).val(),""));                            }                          
                    });     
       ...
       ';
      $form->addElement ('html',NULL,$js);
     $html='<input id="checkall" type="checkbox"> Select / Deselect All <div class="box" style="width:400px; height:150px;">';        
        $array_ZoneActive = explode(",",$arrayActive);
        foreach($aZoneAll as $aZone){    
              if(in_array($aZone['zoneid'],$array_ZoneActive)){
                $html .='<div class="boxrow" id="zone_'.$aZone['zoneid'].'" style="background-color: rgb(255, 255, 255);"><input checked="checked" type="checkbox" value="'.$aZone['zoneid'].'" name="target_zone_'.$aZone['zoneid'].'" class="zone_'.$aZone['zoneid'].'" tabindex="'.$aZone['zoneid'].'">'.$aZone['zonename'].'</div>';
            }else{
                $html .='<div class="boxrow" id="zone_'.$aZone['zoneid'].'" style="background-color: rgb(255, 255, 255);"><input type="checkbox" value="'.$aZone['zoneid'].'" name="target_zone_'.$aZone['zoneid'].'" class="zone_'.$aZone['zoneid'].'" tabindex="'.$aZone['zoneid'].'">'.$aZone['zonename'].'</div>';
            }   
        }
     $html .='</div>';
     $form->addElement ( 'html', 'zone',$html, array ('id' => 'zone' ));
     $form->addElement ( 'hidden', 'target_zone',$arrayActive,array ('id' => 'target_zone' ));
...
function processCampaignForm($form, &$oComponent = null)
{
    ...
    $zone_array=$aFields['target_zone'];
    
    $array_zones = explode(",",$zone_array);
    $aZonesIds = array();
    foreach($array_zones as $array_zone){
        if((!in_array($array_zone, $aZonesIds))&&($array_zone>0)){
            $aZonesIds []= $array_zone;
        }
    }
    ...
    if (!empty($aFields['campaignid']) && $aFields['campaignid'] != "null") {
        $query2 = "
            ...
            From 
                    ox_zones           
            ";
        $rsZones2 = DBC::NewRecordSet($query2);    
        if (PEAR::isError($rsZones2)) {
            return $rsZones2;
        }
        $aZonesAll2 = $rsZones2->getAll();    
        $aZonesIds2 = array();
        foreach ($aZonesAll2 as $aZones2) {
            $aZonesIds2[]= $aZones2;
        }        
        $result = $oDalZones->unlinkZonesFromCampaign($aZonesIds2, $aFields['campaignid']);
        $result2 = $oDalZones->linkZonesToCampaign($aZonesIds, $aFields['campaignid']);
    }
