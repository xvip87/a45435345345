 ...
 * Www_Admin_Plugins_AdflexReport_Lib_Common "interface".
 *
 * @package    OpenXAdmin
 ...
 */
class Www_Admin_Plugins_AdflexReport_Lib_Factory
{
    ...
    function &getController($controllerType = '', $aParams = null)
    {
        ...
        $result = Www_Admin_Plugins_AdflexReport_Lib_Factory::_getControllerClass($controllerType, $aParams, $class, $file);
        ...
        $oStatsController = Www_Admin_Plugins_AdflexReport_Lib_Factory::_instantiateController($file, $class, $aParams);
...
//exit();
    ...
    function _instantiateController($file, $class, $aParams = null)
    {
        ...
        {
            $errMsg = "Www_Admin_Plugins_AdflexReport_Lib_Factory::_instantiateController() Unable to locate " . basename($file);
            return MAX::raiseError($errMsg, MAX_ERROR_INVALIDARGS);
        }
        if (!class_exists($class)) {
            $errMsg = "Www_Admin_Plugins_AdflexReport_Lib_Factory::_instantiateController() Class {$class} doesn't exist";
            return MAX::raiseError($errMsg, MAX_ERROR_INVALIDARGS);
        }
    ...
    function _getControllerClass($controllerType = '', $aParams = null, &$class, &$file)
    {
        ...
        if (!preg_match('/^[a-z-]+$/Di', $controllerType)) {
            $errMsg = "Www_Admin_Plugins_AdflexReport_Lib_Factory::_getControllerClass() Unsupported controller type";
        ...
        $file = MAX_PATH . '/www/admin/plugins/adflexReport/Controller/';
        $file .= $primary;
        foreach ($aSecondary as $string) {
            $file .= $string;
        }
        $file .= '.php';
        $class = 'Www_Admin_Plugins_AdflexReport_Controller_';
        $class .= $primary;
        foreach ($aSecondary as $string) {
            $class .= $string;
        }
