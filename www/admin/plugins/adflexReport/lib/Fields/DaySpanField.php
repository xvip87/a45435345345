 ...
 */
require_once MAX_PATH . '/www/admin/plugins/adflexReport/lib/Field.php';

require_once MAX_PATH . '/lib/OA/Admin/DaySpan.php';

class Admin_UI_DaySpanFieldAdflex extends Admin_UI_FieldAdflex
{
    /* @var string */
     ...
     */
    function Admin_UI_DaySpanFieldAdflex($name = 'DaySpanField',
    ...
    function display()
    {
        ...
        if($user_id>1){
            ...
            $rsClient = DBC::NewRecordSet($query_client);    
            if (PEAR::isError($rsClient)) {
                return $rsClient;
            }        
            $aClientAll = $rsClient->getAll();    
            $client_id=$aClientAll[0];


            $query_campaign = "
                Select
                        clientid,campaignid,campaignname
                From
                        ox_campaigns
                Where
                        clientid=".$aClientAll[0];
            ...
            echo "<script type='text/javascript'>
                function toggle(id) {
                    if(id){
                     window.location.href='adflexReport-index.php?entity=campaign&breakdown=history&clientid=$user_id&campaignid='+id+'&agency_id=$agency_id&advertiser_id=$client_id&period_preset=this_month';
                    }else{
                        window.location.href='adflexReport-index.php?entity=global&breakdown=history';
                    }
                }
            </script> 
        ...
        //print_r($aCampaignAll);
            if(!$_GET['campaignid']){
                echo "<option value=''>All Campaign</option>";
            }else{
                echo "<option selected='selected' value=''>All Campaign</option>";
            }   
        foreach ($aCampaignAll as $aCampaign) {    
            $aCampaignId=$aCampaign['campaignid'];
            $aCampaignName=$aCampaign['campaignname'];
              //$oDalZones  = OA_Dal::factoryDAL('zones');
           //$resultZones = $oDalZones->linkZonesToCampaign($aZonesIds, $aCampaign);
            $selected ="";
            if(($_GET['campaignid'])&&($_GET['campaignid']==$aCampaignId)){
                $selected = " selected='selected'";
            }
            echo "<option value='{$aCampaignId}'{$selected}>$aCampaignName</option>";
        }         

            echo "
            </select>";
        }
        echo "<label for='{$this->_name}_start' style='margin-left: 1em'>$strFrom</label>
