...
function MAX_displayAcls($acls, $aParams) 
{
    ...
    echo '<link href="' . OX::assetPath() . '/css/banner-acl.css" rel="stylesheet" type="text/css"/>';
    echo '<script src="' . OX::assetPath() . '/js/adflex/toggle.js"></script>';
    
    echo "<form action='{$page}' method='post'>";

    echo "<label><span class=\"title\">Banner targeting</span></label>";
    ...
    echo '<ul class="banner-targeting">';
    ...
    foreach ($deliveryLimitations as $pluginName => $plugin) {  
        ...
        if ($plugin->isAllowed($page)) {
            ...
            if(isset($acls[$i])) {
                $plugin = OA_aclGetComponentFromRow($acls[$i]);
                $plugin->init($acls[$i]);
            ...
            $preText = count($acls[$i]['data'])>0?'[-] ':'[+] ';
            echo "<li class='toggle' id='{$pluginName}'>";
            echo '<a href="javascript:void(0)" class="adflex-toggle">' . $preText . $plugin->getName() . '</a>';
            $plugin->count = count($acls);
            $plugin->display();
            echo  "</li>";
           $i++;
        }
    ...
    //echo "<!--/select--></label>";
    echo "&nbsp;";
    echo "<input type='hidden' class='flat' name='action[new]' value='" . $GLOBALS['strAdd'] . "'>";
   
    phpAds_ShowBreak();
    echo "<br />";
    $aErrors = OX_AclCheckInputsFields($acls, $page);
    if (!MAX_AclValidate($page, $aParams)) {
