<?php
/*
File Hook Lib/OA/DLL/Advertiser.php
tuannv add 10/12/2012
*/
...
class Adflex_Register_Advertiser extends OA_Dll
{
    /**
        ...
        if ($this->_validate($oAdvertiser)) {
            ...
            if (!isset($advertiserData['advertiserId'])) {
                ...
                if ($oAdvertiser->advertiserId) {
                    ...
                    $publisherData['agencyid'] = 1;
                    $publisherData['name']  = $oAdvertiser->contactName;
                    $publisherData['contact']  = $oAdvertiser->contactName;
                    $publisherData['comments'] = '';
                    $publisherData['email']    = $this->email;    
                    $publisherData['website'] = 'http://'.$oAdvertiser->websiteUrl;
                    ...
                    if($publisherData['affiliatesid']){
                        $doPublisher = OA_Dal::staticGetDO('affiliates', $publisherData['affiliatesid']);
                        $oPublisher->accountId = (int)$doPublisher->account_id;                  
                    }
					
					
                    if (isset($advertiserData['username']) || isset($advertiserData['userEmail'])) {
                        // Use the authentication plugin to create the user		
                        $oPlugin = OA_Auth::staticGetAuthPlugin();
                        $userId = $oPlugin->getMatchingUserId($advertiserData['userEmail'], $advertiserData['username']);         
                        $userId = $oPlugin->saveUser($userId, $advertiserData['username'], $advertiserData['password'],
                            $advertiserData['contactName'], $advertiserData['userEmail'], $advertiserData['language'], $oAdvertiser->accountId, $advertiserData['userAddress'], $advertiserData['userPhone']);
                        ...
                        if ($userId) {                            
                            ...
                            OA_Permission::setAccountAccess($oAdvertiser->accountId, $userId);
                            OA_Permission::storeUserAccountsPermissions($aPermissions, $oAdvertiser->accountId,
                                $userId, $aAllowedPermissions);
                            
                            OA_Permission::setAccountAccess($oPublisher->accountId, $userId);
                            OA_Permission::storeUserAccountsPermissions($aPermissions, $oPublisher->accountId,
                                $userId, $aAllowedPermissions);                            
                        }
